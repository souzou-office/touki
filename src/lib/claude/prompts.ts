...
## 入力テキストの形式について
入力テキストは以下のいずれかの形式で提供されます:
1. **PDF抽出テキスト**: 登記事項証明書PDFからテキスト抽出したもの。罫線文字（┃━┏┓┗┛┠┨┬┯┴┷┼├─│）でテーブルが構成されています。罫線を無視して、セルの中身のテキストを正確に読み取ってください。
2. **プレーンテキスト**: 手入力またはOCRで取得されたテキスト。

どちらの形式でも、表題部・甲区・乙区の構造を正確に読み取り、上記JSONスキーマに変換してください。
...
- 地積の「：」はコロン区切りではなく小数点を表す（例: 「146：26」は「146.26㎡」を意味する）
- 名前に空白が含まれている場合（例: 「濱 邊 昭 雄」）はスペースを除去して「濱邊昭雄」とする`;
